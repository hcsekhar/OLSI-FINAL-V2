...
def selectbranches(req):
    ...
    return render(req, 'Olsi_app_Html/selectbranches.html',{'data' : prdt_data,'category':cat_data,'categories':categories,'state':state})
...
def branch1(req,id):
    ...
    return render(req,'Olsi_app_Html/branches.html',{'item':items,'state':state})


def vwbranch(req): 
    categories = Branch.objects.all()
    query = req.GET.get('branch1')
    selected = Branches.objects.filter(slug__icontains = query)
    print(selected)
    return render(req,'Olsi_app_Html/branches.html',{'item':selected,'state':state})
